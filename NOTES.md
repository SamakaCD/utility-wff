...
| `ComplicationSlot` cannot be nested in a `ListOption`/`BooleanOption` | Verified on device: the runtime never even attempts to bind a provider to one this way -- zero `WearComplicationProvider` log lines, versus dozens for the same slot declared unconditionally. Every slot has to stay a direct, always-present child of `Scene`, at fixed bounds. Google's own reference confirms this explicitly once you know to look for it. |
| `complicationSlotIds` on a `ListOption` does not hide rendering | It's declarative metadata for the editor (which slots are assignable for that option); on device the "inactive" slots kept drawing, unaffected, right through a clock now sized for the option that was supposed to hide them. If a `UserConfiguration` choice needs to hide what a slot draws, gate the content of each `Complication` with a `Condition` on that configuration's value instead -- see `layout_switch()`/`bulb_slot()`/`bulb_content()` in `gen_wff.py` for the pattern used here (a two-layout switch: gate each Complication's drawn content on `[CONFIGURATION.layout]`, since the bounds and the set of declared slots can't move). |
| Nesting a `Condition` inside another `Condition`'s `Compare` is untested | Rather than risk it, flatten: AND the outer check into the inner `Condition`'s own `Expression`s (`bulb_content()`'s `hasIcon`/`visible` pair), or place two single-level `Condition` elements as siblings instead of one wrapping the other. |
